...
function Object() { return <div className="grid h-full grid-cols-1 items-center gap-8 px-7 md:px-14 lg:grid-cols-[.82fr_1.18fr] lg:px-24"><div className="max-w-lg"><p className="text-xs font-semibold uppercase tracking-[.2em] text-primary">03 — L’objet</p><h2 className="mt-3 font-display text-5xl font-semibold leading-tight">Doux derrière.<br /><span className="italic">Lumineux devant.</span></h2><p className="mt-5 leading-relaxed text-muted-foreground">Signal commence par l’essentiel : un anneau visible de loin, une face calme et une coque arrière en TPU souple. Pas d’écran de plus, pas de compte de plus.</p><div className="mt-6 grid gap-3 sm:grid-cols-2"><Feature title="Un anneau, pas une alarme" copy="La couleur change progressivement pour rester apaisante." /><Feature title="Un dos TPU doux" copy="Une coque souple distincte protège les coins et le mobilier." /></div></div><div className="relative mx-auto h-[47vh] w-full max-w-xl overflow-hidden rounded-[2.75rem] bg-white shadow-soft lg:h-[68vh]"><img src={jasperClairPrototype} alt="Prototype Jasper Clair à écran et coque arrière souple" className="h-full w-full object-cover" /><div className="absolute bottom-5 left-5 rounded-2xl bg-cream/95 px-4 py-3 shadow-soft"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-muted-foreground">Direction suivante</p><p className="mt-1 font-display text-sm font-semibold">Clair · écran + dos TPU</p></div></div></div>; }
...
function Reserve() { const { addItem, openCart } = useCart(); const reserve = () => { addItem(edition.id); openCart(); toast.success("Jasper Signal a été ajouté au panier."); }; return <div className="grid h-full grid-cols-1 items-center gap-8 px-7 md:px-14 lg:grid-cols-[1fr_.85fr] lg:px-24"><div className="max-w-lg"><p className="text-xs font-semibold uppercase tracking-[.2em] text-primary">04 — Première série</p><h2 className="mt-3 font-display text-5xl font-semibold leading-tight">Un seul signal.<br /><span className="italic">Le bon moment.</span></h2><p className="mt-5 leading-relaxed text-muted-foreground">Jasper Signal est la version essentielle : un vrai capteur de CO₂, un anneau lumineux, un dos TPU doux. Les caractéristiques définitives, conformités et conditions de livraison seront confirmées avant l’ouverture commerciale.</p><ul className="mt-6 space-y-2.5 text-sm">{edition.features.map((feature) => <li key={feature} className="flex gap-2.5"><Check className="mt-0.5 h-4 w-4 shrink-0 rounded-full bg-mint p-0.5" />{feature}</li>)}</ul></div><div className="relative rounded-[2.5rem] border border-ink/7 bg-white/90 p-7 shadow-soft md:mr-20"><div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-muted-foreground">Jasper Signal</p><p className="mt-2 font-display text-3xl font-semibold">59 € <span className="ml-1 text-base font-normal text-muted-foreground line-through">69 €</span></p></div><span className="rounded-full bg-mint-soft px-3 py-1.5 text-xs font-semibold">Prix fondateur</span></div><div className="my-6 h-px bg-ink/8" /><p className="text-sm leading-relaxed text-muted-foreground">Une première série pensée pour rendre l’aération évidente, sans écran supplémentaire ni abonnement.</p><button onClick={reserve} className="mt-6 flex w-full items-center justify-center gap-2 rounded-full bg-ink py-3.5 font-semibold text-cream shadow-soft transition hover:scale-[1.01]">Réserver Jasper Signal <ArrowRight className="h-4 w-4" /></button><p className="mt-3 text-center text-[11px] text-muted-foreground">Aucun engagement implicite sur les délais avant publication des conditions.</p></div></div>; }
